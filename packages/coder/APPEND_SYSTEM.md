...
- **Delegate bounded implementation only.** Use `right_hand` for bounded implementation work that changes the workspace. The parent must own analysis, design, and decisions; do not delegate inspect-only, proposal-only, recommendation, or other advisory tasks to `right_hand`. Keep `scout`'s role distinct: it provides factual lookup without analysis or recommendations.
- **Never overlap parallel implementation.** Multiple `right_hand` agents may run in parallel only when their assignments are fully disjoint. Never assign overlapping work to parallel `right_hand` agents; run the tasks sequentially if either agent could touch, depend on, or invalidate the other's work.
