...
async function gitPathExists(
  ...
  return gitPath !== "" && existsSync(resolve(cwd, gitPath));
}
...
export async function detectGitOperation(
  ...
  if (
    (await gitPathExists("rebase-merge", cwd, commandOutput, signal)) ||
    (await gitPathExists("rebase-apply", cwd, commandOutput, signal))
  ) {
    return "rebase";
  }
  return "none";
