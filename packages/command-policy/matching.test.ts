...
suite("flagMatches — flag comparison", () => {
  ...
  test("combined short flag matches constituent single-char", () =>
    assert.ok(flagMatches("-rfv", "-r")));
  test("combined short flag matches constituent multi-char", () =>
    assert.ok(flagMatches("-rfv", "-rf")));
  test("combined short flag matches reversed order", () => assert.ok(flagMatches("-rf", "-fr")));
  test("combined short flag does not match unrelated char", () =>
    assert.ok(!flagMatches("-rfv", "-l")));
  test("no match when completely different flag", () => assert.ok(!flagMatches("-rf", "-x")));
  test("long flag does not spuriously match short flag", () =>
    assert.ok(!flagMatches("--recursive", "-r")));
  test("short flag does not spuriously match long flag", () =>
    assert.ok(!flagMatches("-r", "--recursive")));
});

suite("commandFlags — flag extraction", () => {
  test("extracts flags only", () => {
    const use: CommandUse = {
      name: "git",
      args: ["status", "--short", "-b", "--", "file"],
      segment: "git status --short -b -- file",
    };
...
suite("matchesEntry — command matching", () => {
  test("exact command name", () => {
    ...
    assert.ok(
      matchesEntry(use, { name: "rg", status: CommandPolicyStatus.Allowed, command: "rg" }),
    );
  });
  ...
  test("predicate command match", () => {
    ...
    assert.ok(
      matchesEntry(use, {
        ...
        command: (u) => /^python(?:\d+(?:\.\d+)?)?$/.test(u.name),
      }),
    );
  });
  ...
  test("no subcommand matches any args", () => {
    ...
    assert.ok(
      matchesEntry(use, { name: "git", status: CommandPolicyStatus.Allowed, command: "git" }),
    );
  });
...
suite("findBannedFlag — flag bans", () => {
  ...
  test("detects banned flag in combined short args with git checkout (-fb vs -b)", () => {
    const use: CommandUse = {
      name: "git",
      args: ["checkout", "-fb", "feature"],
      segment: "git checkout -fb feature",
    };
    assert.equal(
      findBannedFlag(use, {
        name: "git checkout",
  ...
  test("flag=value form matches banned flag", () => {
    const use: CommandUse = {
      ...
      segment: "git checkout -b=feature",
    };
    assert.equal(
      findBannedFlag(use, {
        name: "git checkout",
  ...
  test("multiple banned flags returns first match", () => {
    const use: CommandUse = {
      name: "rm",
      args: ["-rf", "--recursive", "dir"],
      segment: "rm -rf --recursive dir",
    };
  ...
  test("no bannedFlags on entry returns null", () => {
    const use: CommandUse = { name: "ls", args: ["-la"], segment: "ls -la" };
    assert.equal(
      findBannedFlag(use, { name: "ls", status: CommandPolicyStatus.Allowed, command: "ls" }),
      null,
    );
  });
});
...
suite("findDisallowedFlag — allowed flags enforcement", () => {
  ...
  test("no allowedFlags on entry returns null", () => {
    const use: CommandUse = { name: "rg", args: ["foo"], segment: "rg foo" };
    assert.equal(
      findDisallowedFlag(use, { name: "rg", status: CommandPolicyStatus.Allowed, command: "rg" }),
      null,
    );
  });
  ...
  test("allowed flag with =value form is accepted", () => {
    const use: CommandUse = {
      ...
      segment: "git status --porcelain=v1",
    };
    assert.equal(
      findDisallowedFlag(use, {
        name: "git status",
...
suite("evaluateCommand — end-to-end policy decision", () => {
  ...
  const entriesWithEcho: CommandPolicyEntry[] = [
    ...testEntries,
